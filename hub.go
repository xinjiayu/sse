...
func (h *hub) run() {
	for {
		select {
		case <-h.shutdown:
			fmt.Println(fmt.Sprintf("hub shutdown requested, cancelling %d connections...", len(h.connections)))
			for c := range h.connections {
				h._shutdownConn(c)
			}
			fmt.Println("...All connections cancelled, shutting down now.")
			return
		case c := <-h.register:
			fmt.Println("new connection being registered for " + c.namespace)
			h.connections[c] = true
		case c := <-h.unregister:
			fmt.Println("connection told us to unregister for " + c.namespace)
			h._unregisterConn(c)
...
func (h *hub) _broadcastMessage(msg SSEMessage) {
	...
	for c := range h.connections {
		if strings.HasPrefix(msg.Namespace, c.namespace) {
			...
			default:
				fmt.Println("cant pass to a connection send chan, buffer is full -- kill it with fire")
				h._shutdownConn(c)
				/*
